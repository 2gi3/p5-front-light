...
const loadCart = () => {
  ...
  let cart_price = 0;
    ...
    products.forEach(product => {
      ...
      fetch(`https://fast-fjord-56594.herokuapp.com/api/products/${product._id} `)
        .then(response => {
...
let email = document.getElementById("email").value;
  ...
  if(validateAll(contact)){
    ...
    fetch('https://fast-fjord-56594.herokuapp.com/api/products/order', options)
    .then(data => {
        if (!data.ok) {
        ...
        window.location.href=window.location.origin+"/confirmation.html?orderId="+response.orderId ;
